...
Word16 nb_of_bits[NUM_OF_MODES] = {
    NBBITS_7k,
    NBBITS_9k,
    NBBITS_12k,
    NBBITS_14k,
...
UWord8 toc_byte[16] = {0x04, 0x0C, 0x14, 0x1C, 0x24, 0x2C, 0x34, 0x3C,
                       0x44, 0x4C, 0x54, 0x5C, 0x64, 0x6C, 0x74, 0x7C};

/* number of speech bits for all modes */
Word16 unpacked_size[16] = {132,  177, 253, 285, 317, 365, 397, 461,
                            477,   35,   0,   0,   0,   0,   0,   0};

/* size of packed frame for each mode, excluding TOC byte */
Word16 packed_size[16] = {17, 23, 32, 36, 40, 46, 50, 58,
                          60,  5,  0,  0,  0,  0,  0,  0};

/* number of unused speech bits in packed format for each mode */
Word16 unused_size[16] = {4, 7, 3, 3, 3, 3, 3, 3, 3, 0, 0, 0, 0, 0, 0, 0};
...
{
    st->sid_update_counter = 3;		// Reset_write_serial
...
void Write_serial(Word16 prms[], Word16 coding_mode, Word16 mode, TX_State *st, Word16 bitstreamformat)
{
    ...
    *stream_ptr++ = toc_byte[coding_mode];
    ...
    for (i = 0; i < unpacked_size[coding_mode]; i++)
    {
        if (prms[sort_ptr[coding_mode][i]] == BIT_1)
   	    {
            temp++;
        }
...
{
    st->prev_ft = RX_SPEECH_GOOD;		// Reset_read_serial
    st->prev_mode = 0;
...
/************************************************************************\
 *                                                                      *
 *  // read speech bits, return with empty frame if mismatch between
 *  //    mode info and available data
...
\************************************************************************/
    ...
    for (i = 0; i < unpacked_size[*mode]; i++)
    {
		prms[sort_ptr[*mode][i]] = _bits[(temp >> 7)&1];
        if ((i + 1) % 8)
            temp <<= 1;
        else
            temp = *packet_ptr++;
