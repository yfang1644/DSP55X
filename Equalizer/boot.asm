;*****************************************************************************
;* BOOT.ASM   v4.3.8 - Initialize the TMS320C55x C/C++ runtime environment
...
ST2_OFF  .equ (1<<ST2_RDM) | (1<<ST2_CDPLC) | ST2_03LC | ST2_47LC
  ...
  ;    SST    == 0  No saturation-on-store in C54x mode
  ;    others == don't care
  ...
  ; Change the bits
	...
	@ST1_L = @ST1_L | #( ST1_ON ) || mmap()
     ...
     ; ATTENTION! If the runtime initialization sequence is modified by the
     ; user to execute with interrupts enabled at this point then the following
  ...
  ; and some have different names (SMUL => GSM, C54CM => LEAD) or do not
  ; exist (SST)
...
;------------------------------------------------------------------------
  .if .PHASE2
	...
	MDP05 = #0
	MDP67 = #0
  .elseif (! __LARGE_MODEL__ & ! __HUGE_MODEL__)
	XAR0 = #___bss__
	XAR1 = XAR0
	XAR2 = XAR0
	XAR3 = XAR0
