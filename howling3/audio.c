...
Int16 rcv_l[BUFFER*2], rcv_r[BUFFER*2];
Int16 xmt_l[BUFFER*2], xmt_r[BUFFER*2];
...
Int16 hilbert[] = { /* 31 taps, x32768 */
    111, 0, 192, 0, 440, 0, 922, 0, 1752, 0, 3212, 0, 6342, 0, 20651, 0,
  -20651, 0, -6342, 0, -3212, 0, -1752, 0, -922, 0, -440, 0, -192, 0, -111};

short FIR_filter(short input, short *delay)
{
    static short value[TAPS];	// Retain value between calls
    int i;                      // Index into filter constants
    long product;
    static  int j = 0;          // Index into input array

    product = 0;

    for (i = 0; i < TAPS; i += 2) {
        product += ((long)value[j] * hilbert[i]) >> 3;
        j += 2;
        if (j >= TAPS)    j -= TAPS;
    }
	...
	*delay = value[i];
    ...
    product >>= 12;     // Second part of divide by 32768
    if(++j >= TAPS) j = 0;
...
#define	TBLNUM			12		/* sine table size = 2^12 */
...
void audioProcessing(int func)
{
	...
	offset = ((pingpong>>1) & 1) * BUFFER;
	if(func == 0) {
		DSP_memcpy(&xmt_l[offset], &rcv_l[offset], BUFFER);
		DSP_memcpy(&xmt_r[offset], &rcv_r[offset], BUFFER);
		return;
	}

	for(i = 0; i < BUFFER; i++) {
		sinPtr &= (4*SINETBLSIZE-1);
		quad = sinPtr >> TBLNUM;
		ptr = sinPtr & (SINETBLSIZE-1);
		switch(quad) {
		case 0:
			s = sine_table[ptr];
			c = (ptr == 0) ? 0x7fff : sine_table[SINETBLSIZE - ptr];
			break;
		case 1:
			s = (ptr == 0) ? 0x7fff : sine_table[SINETBLSIZE - ptr];
			c = -sine_table[ptr];
			break;
		case 2:
			s = -sine_table[ptr];
			c = (ptr == 0) ? 0x8000 : -sine_table[SINETBLSIZE - ptr];
			break;
		case 3:
			s = (ptr == 0) ? 0x8000 : -sine_table[SINETBLSIZE - ptr];
			c = sine_table[ptr];
			break;
		default:
			break;
		...
		ch1 = FIR_filter(rcv_l[offset + i], &ch2);
		sum  = (long)ch1 * s;
		sum += (long)ch2 * c;
		ch1 = (short)(sum >> 15);
		xmt_l[offset + i] = ch1;
