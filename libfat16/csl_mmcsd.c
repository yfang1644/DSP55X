...
void DSP_word2byte(Uint8 *dst, Uint16 *src, int cnt);
void DSP_byte2word(Uint16 *dst, Uint8 *src, int cnt);
...
{
	return MMC_read(mmcsdHandle, (sect<<9), 512, (Uint16 *)p);
}

int SD_WriteBlock_Raw(Uint32 sect, Uint8 *p)
{
	return MMC_write(mmcsdHandle, (sect<<9), 512, (Uint16 *)p);
}
