...
/* local function */
void add_pulses(Word16 pos[], Word16 nb_pulse, Word16 track, Word16 code[])
{
    Word16 i, k;
...
)
{
    ...
    case 64:
        for (k = 0; k < NB_TRACK; k++)
        {
            L_index = L_add((Word32)index[k] << 14, index[k + NB_TRACK]);
    ...
    case 72:
        for (k = 0; k < NB_TRACK - 2; k++)
        {
            L_index = L_add((Word32)index[k] << 10, index[k + NB_TRACK]);
    ...
    case 88:
        for (k = 0; k < NB_TRACK; k++)
        {
            L_index = L_add((Word32)index[k] << 11, index[k + NB_TRACK]);
