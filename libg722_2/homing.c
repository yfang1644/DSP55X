...
Word16 encoder_homing_frame_test(Word16 input_frame[])
{
    ...
    /* check 320 input samples for matching EHF_MASK: defined in cnst.h */
...
Word16 dhf_test(Word16 input_frame[], Word16 mode, Word16 nparms)
{
    ...
    if (mode != MRDTX)
    {
        ...
        else
        {
            ...
            tmp = nparms;
			for(i = 0; tmp > 15; i++, tmp -= 15)
            {
                param[i] = Serial_parm(15, &prms);
            }
            param[i] = Serial_parm(tmp, &prms);
            shift = 15 - tmp;
            param[i] = param[i] << shift;
        }
