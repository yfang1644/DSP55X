...
Int16 index_to_array(Int16 index, Int16 *array, Int16 category)
{
    ...
    max_bin_1 = max_bin_plus_one[category];
    inverse_of_max_bin_plus_one = max_bin_plus_one_inverse[category];

    for (j = vector_dimension[category]; j--; ) 
    {
        q = mult(p, inverse_of_max_bin_plus_one);
        temp = (Int16)((Int32)q * (Int32)max_bin_1);
        array[j] = sub(p, temp);
					                     ...
					                     Int16   *decoder_mlt_coefs)
{
    ...
    for (region = 0; region < gNumber_of_regions; region++) 
    {
        ...
        if (category < 7)
        {
            ...
            {
        	    temp = add(region, 1);
                for (j = temp; j < gNumber_of_regions; j++)
                {
                         ...
                         Int16 *absolute_region_power_index)
{
    ...
    {
        if (categorization_control < gNum_categorization_control_possibilities - 1)
        {
            if (bitobj->number_of_bits_left < 0)
            {
                *frame_error_flag |= 2;
...
***************************************************************************/
Int16 old_decoder_mlt_coefs[MAX_DCT_LENGTH];
             ...
             Int16 *p_old_mag_shift)
{
    ...
    Int16  decoder_power_categories[MAX_NUMBER_OF_REGIONS];
    Int16  decoder_category_balances[MAX_NUM_CATEGORIZATION_CONTROL_POSSIBILITIES];
    Int16  decoder_region_standard_deviation[MAX_NUMBER_OF_REGIONS];
    Int16  i;
	...
	number_of_coefs = gDct_length;
	number_of_valid_coefs = gNumber_of_valid_coefs;
    ...
    for (i = 0; i < gNum_categorization_control_bits; i++) 
    {
        get_next_bit(&bitobj);
	...
	bitobj.number_of_bits_left -= gNum_categorization_control_bits;
