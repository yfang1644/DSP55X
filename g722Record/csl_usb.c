...
CSL_Status USB_init(CSL_UsbConfig    *usbConfig)
{
	...
	*(volatile ioport Uint16 *)(0x7004) = 0x0001; 
	...
	{
		sysCtrlRegs  = (CSL_SysRegsOvly)CSL_SYSCTRL_REGS;
		...
		/* Must wait some time, elsewhere no suspend interrupt */
...
CSL_Status USB_resetDev(CSL_UsbDevNum    devNum)
{
    ...
    /* enable ep0 intr */
	CSL_FINS(usbRegisters->INTRTXE, USB_INTRTXE_EP0, TRUE);
	/* enable ep1 intr */
	CSL_FINS(usbRegisters->INTRTXE, USB_INTRTXE_EP1TX, TRUE);
	/* enable ep2 intr*/
	CSL_FINS(usbRegisters->INTRRXE, USB_INTRRXE_EP2RX, TRUE);
    /* enable ep3 intr*/
    CSL_FINS(usbRegisters->INTRTXE, USB_INTRTXE_EP3TX, TRUE);
	...
	if(hEp != NULL)
	{
		...
		if((peps != NULL) && (peps->fInitialized == TRUE))
		{
			...
			if(peps->dwEndpoint == CSL_USB_EP0) {
				...
				if(hEp->epNum > CSL_USB_IN_EP0) { /* Stall Tx Ep */
					CSL_FINS(usbRegisters->PERI_CSR0_INDX,
