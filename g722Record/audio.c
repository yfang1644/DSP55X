	...
	case FILE_REPLAY_RAW:
		for(chn = 0; chn < STEREO; chn++) {
			...
			if(recordFile.channel & (1 << chn)) {	// REPLAY chn
				readFileProcessing(chn);
				DSP_byte2word(samples,
...
void Audio_init(Uint16 bandwidth)
{
	...
	recordFile.channel = 1;		/* only left channel encoding */
	recordFile.code = 0x0116;		/* 01=g722.1, 16=16kbps */
