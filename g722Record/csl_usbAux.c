                                ...
                                Uint32                 *pLinkingRam)
{
	...
	u32Addr   = ((Uint32)pLinkingRam) << 1;
	u16CpyReg = (Uint16)(u32Addr & CSL_USB_16BIT_REG_MASK);

	// disable the hardware interrupt to make sure the 
	// following operations are atomic  
//	value = IRQ_globalDisable();
	usbRegisters->LRAM0BASE1 = u16CpyReg;
	u16CpyReg = (Uint16)((u32Addr >> CSL_USB_16BIT_REG_SHIFT) &
...
CSL_Status USB_processEP0Out(pUsbContext    pContext)
{
    ...
    if(pContext != NULL)
    {
		...
		if((pTransfer != NULL) && (pTransfer->fComplete == FALSE))
		{
			...
			for(i = (pTransfer->cbTransferred+1)/2; i--;)
				pwBuffer[i] = pContext->dwOutEP0Buf[i];
