...
Int16 rcv_l[0x2000], xmt_l[FFT2*2];
Int16 rcv_r[0x2000], xmt_r[FFT2*2];
...
void audioProcessing(void)
{
	...
	offset = ((pingpong>>1) & 1) * FFT2;

	DSP_memcpy(&xmt_l[offset], &rcv_l[offset], FFT2);
	DSP_memcpy(&xmt_r[offset], &rcv_r[offset], FFT2);
...
void disableDMA(void)
{
	...
	CSL_CPU_REGS->IER0 &= ~0x0100;
	EZDSP5535_I2S_init(1);		/* Initialize I2S */
}
