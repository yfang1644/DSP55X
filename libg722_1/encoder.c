                   ...
                   Int16  number_of_bits_per_frame)
{
    ...
    drp_num_bits[gNumber_of_regions] = gNum_categorization_control_bits;

    drp_code_bits[gNumber_of_regions] = (Uint16)categorization_control;
...
//        acca = (Int32)power_shift - acca;
    ...
    for (region = gNumber_of_regions - 2; region >= 0; region--)
    {
    ...
    for (region = 1; region < gNumber_of_regions; region++)
    {
                      ...
                      Uint32 *word_ptr)
{
    ...
    Uint32 current_word = 0L;
    Int16  current_word_bits_free = 32;
    ...
    kmax_plus_one = max_bin_plus_one[category];
    kmax = kmax_plus_one - 1;
    ...
    acca = L_mac(8192,
                 step_size_inverse_table[category],
    ...
    for (n = 0; n < num_vecs; n++)
    {
        ...
        for (j = 0; j < vec_dim; j++)
        {
            ...
            acca = L_mac((Int32)int_dead_zone[category],
						  k, inv_of_step_size_times_std_dev);

			myacca = L_mac((Int32)int_dead_zone_low_bits[category],
						  k, mytemp);
			myacca = myacca >> 2;
            ...
            if (k != 0)
            {
                number_of_non_zero++;		/* not exceed than vec_dim(5) */
                          ...
                          Uint32 *region_mlt_bits)
{
    ...
    temp = gNum_categorization_control_possibilities >> 1;
    temp = temp - 1;
    ...
    while ((total_mlt_bits > number_of_available_bits) &&
           (*p_categorization_control < gNum_categorization_control_possibilities - 1))
    {
             ...
             Int16  *out_words)
{
    ...
    Int16  categorization_control;
    Int16  region;
    Int16  absolute_region_power_index[MAX_NUMBER_OF_REGIONS];
    Int16  power_categories[MAX_NUMBER_OF_REGIONS];
    Int16  category_balances[MAX_NUM_CATEGORIZATION_CONTROL_POSSIBILITIES];
    Int16  drp_num_bits[MAX_NUMBER_OF_REGIONS+1];
    Uint16 drp_code_bits[MAX_NUMBER_OF_REGIONS+1];
    Int16  region_mlt_bit_counts[MAX_NUMBER_OF_REGIONS];
    Uint32 region_mlt_bits[4*MAX_NUMBER_OF_REGIONS];
    ...
    number_of_available_bits = sub(temp, gNum_categorization_control_bits);
