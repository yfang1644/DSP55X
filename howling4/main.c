...
*/
	...
	aic3204_init(3, 40);		/* Initialize codec, sampling rate = 32kHz */
    DMA_audio_init(FFT_L);
	...
	while(1) {
		audioProcessing(active);

		key = EZDSP5535_SAR_getKey();
		if(key == SW1) {
			active = 1;
			DSP_zero(dbuff, FFT_L);
			DSP_zero(aflt, 2* FFT_L);
			DSP_zero(rcv_l, 2* FFT_L);
