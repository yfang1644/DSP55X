...
/*	VECS1	(RWIX): origin = 0x010000, length = 0x000100  */
	...
	SARAM2	(RWIX): origin = 0x040000, length = 0x00FC00
/*	VECS0	(RWIX): origin = 0x04FF00, length = 0x000100 */
	PDROM	 (RIX): origin = 0xff8000, length = 0x008000
	VECS2	(RWIX): origin = 0x010000, length = 0x000100
...
SECTIONS
{
   ...
   /* Both stacks must be on same physical memory page               */
	...
	.args     >  SARAM2                /* Arguments to main()         */
	 dmaMem   >  DARAM0
    .twiddle  >  SARAM2                /*FFT-specific sections  */
	.input    >  SARAM2, align(4)      /* (n+1) LSBs should be 0, while n=log2(nx) */
	.vectors  >  VECS2                 /* Interrupt vectors           */
/*	 vectors  >  VECS0 */
