 ...
 */
static
mad_fixed_t I_sample(struct mad_bitptr *ptr, unsigned short nb)
{
  mad_fixed_t sample;
 ...
 */
short mad_layer_I(struct mad_stream *stream, struct mad_frame *frame)
{
  struct mad_header *header = &frame->header;
  unsigned short nch, bound, ch, s, sb, nb;
...
{
  unsigned short nb, s, sample[3], nlevels;
  unsigned long c;
 ...
 */
short mad_layer_II(struct mad_stream *stream, struct mad_frame *frame)
{
  struct mad_header *header = &frame->header;
  struct mad_bitptr start;
  unsigned short index, sblimit, nbal, nch, bound, gr, ch, s, sb;
  unsigned char const *offsets;
  unsigned char allocation[2][32], scfsi[2][32], scalefactor[2][32][3];
  unsigned short bitrate_per_channel;
  ...
  if (header->flags & MAD_FLAG_LSF_EXT)
    ...
    index = (header->samplerate == 48) ? 0 : 1;
  else {
...
# if defined(OPT_STRICT)
       ...
       */
      if (bitrate_per_channel <= 28 || bitrate_per_channel == 40) {
...
# endif
    ...
    if (bitrate_per_channel <= 48)
      index = (header->samplerate == 32) ? 3 : 2;
