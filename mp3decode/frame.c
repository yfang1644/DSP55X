...
static
unsigned short const bitrate_table[5][15] = {	/* bitrate in kbps */
  /* MPEG-1 */
...
static
short (*const decoder_table[3])(struct mad_stream *, struct mad_frame *) = {
  mad_layer_I,
  mad_layer_II,
  mad_layer_III
};
 ...
 */
void DSP_zero(void *dst, int cnt);
void mad_frame_mute(struct mad_frame *frame)
{
	DSP_zero(frame->sbsample, 2*32*36);
	DSP_zero(frame->overlap, 2*32*18);
}
...
int free_bitrate(struct mad_stream *stream, struct mad_header const *header)
{
  ...
  unsigned short pad_slot, slots_per_frame;
  ...
  while (mad_stream_sync(stream) == 0) {
	...
	rate = (unsigned short)(header->samplerate * (N - 4 * pad_slot + 4) / 48 / 1000);
      }
      else {
	rate = (unsigned short)(header->samplerate * (N - pad_slot + 1) / slots_per_frame / 1000);
      }
 ...
 sync:
  ...
  if (header->bitrate == 0) {
    if ((stream->freerate == 0 || !stream->sync ||
	 (header->layer == MAD_LAYER_III && stream->freerate > 640)) &&
  ...
  if (header->layer == MAD_LAYER_I)
    N = ((1000L * 12 * header->bitrate / header->samplerate) + pad_slot) * 4;
  else {
    slots_per_frame = (header->layer == MAD_LAYER_III &&
		       (header->flags & MAD_FLAG_LSF_EXT)) ? 72 : 144;

    N = (1000L * slots_per_frame * header->bitrate / header->samplerate) + pad_slot;
  }
