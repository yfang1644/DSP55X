 ...
 */
static
void dct32(mad_fixed_t const in[32], unsigned short slot,
...
/*
 * NAME:	synth->full()
 * DESCRIPTION:	perform full/half frequency PCM synthesis
 */
static
void synth_full(struct mad_synth *synth, struct mad_frame const *frame,
		unsigned short nch, unsigned short ns, short full)
{
  unsigned short phase, ch, s, sb, pe, po;
  ...
  for (ch = 0; ch < nch; ++ch) {
    ...
    for (s = 0; s < ns; ++s) {
      ...
      /* calculate 32 samples (full) or 16 samples(half)*/
...
void mad_synth_frame(struct mad_synth *synth, struct mad_frame const *frame)
{
  ...
  synth_full(synth, frame, nch, ns,full);

  synth->phase = (synth->phase + ns) & 15;
}
