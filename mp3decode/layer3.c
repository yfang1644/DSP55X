...
struct sideinfo {
  ...
  unsigned short scfsi[2];

  struct granule {
    struct channel {
      /* from side info */
      short part2_3_length;
 ...
 */
static
enum mad_error III_sideinfo(struct mad_bitptr *ptr, unsigned short nch,
			    short lsf, struct sideinfo *si,
			    unsigned short *data_bitlen,
			    unsigned short *priv_bitlen)
{
  unsigned short ngr, gr, ch, i;
 ...
 */
static
unsigned long III_scalefactors_lsf(struct mad_bitptr *ptr,
				  struct channel *channel,
				  struct channel *gr1ch, short mode_extension)
{
  struct mad_bitptr start;
  unsigned short scalefac_compress, index, slen[4], part, n, i;
  unsigned char const *nsfb;

  unsigned long max, is_pos;
 ...
 */
static
unsigned long III_scalefactors(struct mad_bitptr *ptr, struct channel *channel,
			      struct channel const *gr0ch, unsigned short scfsi)
{
  struct mad_bitptr start;
  unsigned short slen1, slen2, sfbi;
  unsigned short nsfb;
...
void III_exponents(struct channel const *channel,
		   unsigned char const *sfbwidth, short exponents[39])
{
  short gain;
  unsigned short scalefac_multiplier, sfbi;
  unsigned short premask;

  gain = channel->global_gain - 210;
  ...
  if (channel->block_type == 2) {
    ...
    if (channel->flags & mixed_block_flag) {
      ...
      while (l < 36) {
        exponents[sfbi] = gain -
	      ((channel->scalefac[sfbi] + (pretab[sfbi] & premask)) <<
             scalefac_multiplier);
    ...
    gain0 = gain - 8 * channel->subblock_gain[0];
    gain1 = gain - 8 * channel->subblock_gain[1];
    gain2 = gain - 8 * channel->subblock_gain[2];

    while (l < 576) {
      exponents[sfbi + 0] = gain0 -
	(channel->scalefac[sfbi + 0] << scalefac_multiplier);
      exponents[sfbi + 1] = gain1 -
	(channel->scalefac[sfbi + 1] << scalefac_multiplier);
      exponents[sfbi + 2] = gain2 -
	(channel->scalefac[sfbi + 2] << scalefac_multiplier);
 ...
 */
static
mad_fixed_t III_requantize(unsigned short value, short exp)
{
  ...
  if (exp < 0) {
    if (-exp >= sizeof(mad_fixed_t) * 2 * CHAR_BIT) {
      /* underflow */
      requantized = 0;
    }
    else {
...
enum mad_error III_huffdecode(struct mad_bitptr *ptr, mad_fixed_t xr[576],
			      ...
			      unsigned long part2_length)
{
  short exponents[39], exp;
  short const *expptr;
  struct mad_bitptr peek;
  short bits_left, cachesz;
  mad_fixed_t *xrptr;
  mad_fixed_t const *sfbound;
  unsigned long bitcache;
  mad_fixed_t req_x, req_y;
  ...
  /* big_values */
  {
    unsigned short region, rcount;
    struct hufftable const *entry;
    union huffpair const *table;
    unsigned short linbits, startbits, big_values, reqhits;
	...
	x_final:
	  req_x = MASK1BIT(bitcache, cachesz--) ?
	...
	y_final:
	  req_y = MASK1BIT(bitcache, cachesz--) ?
	    -requantized : requantized;
	}
      }
      else {
  ...
  /* count1 */
  {
    ...
    while (cachesz + bits_left > 0 && xrptr < &xr[576]) {
		 ...
		 unsigned char const sfbwidth[39])
{
  ...
  unsigned short sb, l, f, w, sbw[3], sw[3];
  ...
  mem_cpy(&xr[18 * sb], &tmp[sb], (576 - 18 * sb) * sizeof(mad_fixed_t));
}
			  ...
			  unsigned char const *sfbwidth)
{
  ...
  if (header->mode_extension & I_STEREO) {
    ...
    if (right_ch->block_type == 2) {
      unsigned short lower, start, max, bound[3], w;
 ...
 */
static
void III_aliasreduce(mad_fixed_t xr[576], short lines)
{
...
void fastsdct(mad_fixed_t const x[9], mad_fixed_t y[18])
{
  ...
  mad_fixed_t c0 =  MAD_F(0x1f838b8d);  /* 2 * cos( 1 * PI / 18) */
  mad_fixed_t c1 =  MAD_F(0x1bb67ae8);  /* 2 * cos( 3 * PI / 18) */
  mad_fixed_t c2 =  MAD_F(0x18836fa3);  /* 2 * cos( 4 * PI / 18) */
  mad_fixed_t c3 =  MAD_F(0x1491b752);  /* 2 * cos( 5 * PI / 18) */
  mad_fixed_t c4 =  MAD_F(0x0af1d43a);  /* 2 * cos( 7 * PI / 18) */
  mad_fixed_t c5 =  MAD_F(0x058e86a0);  /* 2 * cos( 8 * PI / 18) */
  mad_fixed_t c6 = -MAD_F(0x1e11f642);  /* 2 * cos(16 * PI / 18) */
...
void dctIV(mad_fixed_t const y[18], mad_fixed_t X[18])
{
  ...
  for (i = 1; i < 18; i++)  X[i] -= X[i - 1];
  for (i = 0; i < 18; i++)  X[i] >>= 1;
}
...
# endif
  ...
  case 1:  /* start block */
    for (i =  0; i < 18; i++) z[i] = mad_f_mul(z[i], window_l[i + 0]);
  ...
  case 3:  /* stop block */
    ...
    for (i = 18; i < 36; i++) z[i] = mad_f_mul(z[i], window_l[i + 0]);
    break;
  }
}
# endif  /* ASO_IMDCT */
...
void III_overlap(mad_fixed_t const output[36], mad_fixed_t overlap[18],
		 mad_fixed_t sample[18][32], unsigned short sb)
{
  unsigned short i;
  for (i = 0; i < 18; ++i) {
 ...
 */
static
void III_freqinver(mad_fixed_t sample[18][32], unsigned short sb)
{
	int i;
  for (i = 1; i < 18; i += 2)
...
enum mad_error III_decode(struct mad_bitptr *ptr, struct mad_frame *frame,
			  struct sideinfo *si, unsigned short nch)
{
  struct mad_header *header = &frame->header;
  unsigned long sfreqi, ngr, gr;
  unsigned long sfreq;
  unsigned long part2_length;
 ...
 */
short mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  struct mad_header *header = &frame->header;
  unsigned short nch, priv_bitlen, next_md_begin = 0;
  unsigned short si_len, data_bitlen, md_len;
  unsigned short frame_space, frame_used, frame_free;
  ...
  if (si.main_data_begin == 0) {
	...
	mem_cpy(stream->main_data + stream->md_len,
