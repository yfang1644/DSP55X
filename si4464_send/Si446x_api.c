...
void SetPAMode(Uint8 bPaMode, Uint8 bModType)
{
	...
	SpiSendCommand(8, abApi_Write, 1);
}
...
void bApi_Set_Receive(void)					//将系统设置到接收状态
{
	...
	SpiSendCommand(2, abApi_Write, 1);
	...
	abApi_Write[6] = 0x05;					// Ready state after Rx valid state
	// 05 TX tune State ; 03 ready state
	// 接收完后用于减少电流用03,需要较高的响应速度用05
	abApi_Write[7] = 0x05;					// Ready state after Rx invalid state
	SpiSendCommand(8, abApi_Write, 1);
}
...
void bApi_Set_Send(Uint16 length)				//将系统设置到发射状态
{
	...
	SpiSendCommand(5, abApi_Write, 1);

	SpiWriteByte(CMD_GET_INT_STATUS, 0);		// clear radio IC interrupt
...
void ReceivePacket(Uint8 *buff)
{
	...
	SpiGetResponse(CMD_FRR_C_READ, 1, abApi_Read);	// read out latched RSSI

	SpiWriteByte(CMD_FIFO_INFO, 0);
	SpiGetResponse(CMD_READ_CMD_BUFF, 2, abApi_Read);
  ...
  // Provides access to transmit and receive fifo counts and reset, clear RXFIFO  
	SpiWriteByte(CMD_FIFO_INFO, 3);			// clear Tx, Rx FIFO
...
void Si446x_Init(void)
{
	...
	abApi_Write[1] = 0x01;
	abApi_Write[2] = 0x00;
	SpiSendCommand(3, abApi_Write, 1);

	abApi_Write[0] = CMD_PART_INFO;
	SpiSendCommand(1, abApi_Write, 0);
	...
	abApi_Write[2] = 0;							// Clear MODEM_CLR_PEND
	abApi_Write[3] = 0;							// Clear CHIP_CLR_PEND
	SpiSendCommand(4, abApi_Write, 0);
	SpiGetResponse(CMD_READ_CMD_BUFF, 9, abApi_Read);

	// Set TRX parameters of the radio IC;
	//data retrieved from the WDS-generated modem_params.h
	SpiSendCommand(ModemTrx1[0], (Uint8 *)&ModemTrx1[1], 1);
	SpiSendCommand(ModemTrx2[0], (Uint8 *)&ModemTrx2[1], 1);
	SpiSendCommand(ModemTrx3[0], (Uint8 *)&ModemTrx3[1], 1);
	SpiSendCommand(ModemTrx4[0], (Uint8 *)&ModemTrx4[1], 1);

	// Set Rx parameters of the radio IC
	SpiSendCommand(ModemRx1[0], (Uint8 *)&ModemRx1[1], 1);
	SpiSendCommand(ModemRx2[0], (Uint8 *)&ModemRx2[1], 1);
	SpiSendCommand(ModemRx3[0], (Uint8 *)&ModemRx3[1], 1);
	SpiSendCommand(ModemRx4[0], (Uint8 *)&ModemRx4[1], 1);
	SpiSendCommand(ModemRx4_1[0], (Uint8 *)&ModemRx4_1[1], 1);
	SpiSendCommand(ModemRx5[0], (Uint8 *)&ModemRx5[1], 1);
	SpiSendCommand(ModemRx6[0], (Uint8 *)&ModemRx6[1], 1);
	SpiSendCommand(ModemRx7[0], (Uint8 *)&ModemRx7[1], 1);
	SpiSendCommand(ModemRx8[0], (Uint8 *)&ModemRx8[1], 1);
	SpiSendCommand(ModemRx9[0], (Uint8 *)&ModemRx9[1], 1);
	SpiSendCommand(ModemRx10[0], (Uint8 *)&ModemRx10[1], 1);
	SpiSendCommand(ModemRx11[0], (Uint8 *)&ModemRx11[1], 1);
	SpiSendCommand(ModemRx12[0], (Uint8 *)&ModemRx12[1], 1);

	// Set TX parameters of the radio IC
	SpiSendCommand(ModemTx1[0], (Uint8 *)&ModemTx1[1], 1);
	SpiSendCommand(ModemTx2[0], (Uint8 *)&ModemTx2[1], 1);
	...
	SpiSendCommand(7, abApi_Write, 1);
	...
	abApi_Write[4] = 0x00;						// PKT_FIELD_1_LENGTH_12_8: defined later (under SendPacket() )
	abApi_Write[5] = 0x00;						// PKT_FIELD_1_LENGTH_7_0: defined later (under SendPacket() )
